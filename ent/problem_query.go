...
func (pq *ProblemQuery) QueryJudge() *JudgeQuery {
	query := (&JudgeClient{config: pq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (pq *ProblemQuery) loadSubmissions(ctx context.Context, query *SubmissionQuery, nodes []*Problem, init func(*Problem), assign func(*Problem, *Submission)) error {
	...
	for _, n := range neighbors {
		...
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "problem_submissions" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (pq *ProblemQuery) loadJudge(ctx context.Context, query *JudgeQuery, nodes []*Problem, init func(*Problem), assign func(*Problem, *Judge)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Problem)
	for i := range nodes {
		if nodes[i].judge_problems == nil {
			continue
		}
		fk := *nodes[i].judge_problems
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "judge_problems" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
