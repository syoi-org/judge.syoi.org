...
func (c *Client) noder(ctx context.Context, table string, id int) (Noder, error) {
	...
	case judge.Table:
		...
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, judgeImplementors...); err != nil {
				return nil, err
			}
		}
	...
	case problem.Table:
		query := c.Problem.Query().
			Where(problem.ID(id))
		if fc := graphql.GetFieldContext(ctx); fc != nil {
			if err := query.collectField(ctx, true, graphql.GetOperationContext(ctx), fc.Field, nil, problemImplementors...); err != nil {
				return nil, err
			}
		}
		return query.Only(ctx)
...
func (c *Client) noders(ctx context.Context, table string, ids []int) ([]Noder, error) {
	...
	case judge.Table:
		query := c.Judge.Query().
			Where(judge.IDIn(ids...))
		query, err := query.CollectFields(ctx, judgeImplementors...)
		if err != nil {
			return nil, err
		}
	...
	case problem.Table:
		...
		query, err := query.CollectFields(ctx, problemImplementors...)
		if err != nil {
			return nil, err
		}
		nodes, err := query.All(ctx)
